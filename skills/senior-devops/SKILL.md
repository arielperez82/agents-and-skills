...
4. **Test Pipeline Locally** - Use `act` to validate workflow changes before committing:
   ```bash
   ...
   ```
   **When to use `act`:** For workflow changes that don't have production side-effects (syntax, environment variables, script changes, step ordering). Always test locally before committing workflow changes.
5. **Test Pipeline in CI** - Trigger test run, validate all stages execute correctly
6. **Enable Branch Protection** - Require pipeline success before merging
