...
- **act:** Local GitHub Actions testing (see Local Testing section below)

## Local Workflow Testing with `act`

### When to Test Locally

Use `act` to validate GitHub Actions workflow changes **before committing** when:
- Modifying workflow syntax or structure
- Changing environment variable extraction
- Updating script paths or commands
- Reordering steps or jobs
- Adding new workflow steps
- Testing workflow logic without production side-effects

**Do NOT use `act` for:**
- Workflows that deploy to production
- Workflows that create external resources
- Workflows that modify production data
- Workflows with side-effects that can't be safely tested locally
...
# Test entire workflow (simulates push event)
...
act -j <job-name> --env-file .env.local --container-architecture linux/amd64
```
...
```bash
act -j <job-name> --container-architecture linux/amd64 -v
```

**Test specific workflow file:**
```bash
act -W .github/workflows/ci.yml -j <job-name> --container-architecture linux/amd64
```

### Limitations

- `act` runs workflows in Docker containers, which may behave slightly differently than GitHub-hosted runners
- Some GitHub Actions may not work perfectly in local environment
- Secrets and environment variables need to be provided manually
- Network access and external services may behave differently

### Best Practices

1. **Always test workflow changes locally** before committing
2. **Use `--container-architecture linux/amd64`** to match GitHub runners
3. **Test the specific job** you're modifying rather than entire workflow
4. **Verify environment variables** are correctly extracted and passed
5. **Check script execution** paths and permissions
6. **Validate workflow syntax** with `act` even if YAML is valid
