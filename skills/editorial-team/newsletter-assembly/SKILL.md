...
- **Manual Section Discovery** — Scan templates for `(manual)` markers, extract metadata (source, auto-brief, agent), and present provide/generate/skip options per section
...
Templates can mark sections as `(manual)` — content that normally comes from external sources (Google Docs, platform data, show notes) rather than being auto-generated by the pipeline. This capability extracts structured metadata about each manual section so the command layer can ask the user what to do with each one.

### How It Works

1. **Scan the template** for `## ... (manual)` headings (e.g., `## Nostalgia Nerd (manual)`, `## SPONSOR (manual)`)
2. **Cross-reference** each manual section with the template's "Section Guidance" and "Manual Sections" list to extract:
   - `name`: The section heading (without the `(manual)` marker)
   - `source`: Where the content normally comes from (e.g., "external Google Doc", "show notes", "platform data")
   - `auto-brief`: The agent brief for auto-generation, if the template provides one (e.g., Nostalgia Nerd has a `Brief for editorial-writer`)
   - `auto-agent`: Which agent to dispatch if auto-generating (derived from the brief attribution)
   - `required-input`: What the human would need to provide if choosing to supply content manually
3. **Return a structured list** of manual sections with their metadata, feeding into the command's dynamic questioning step

### Resolution Options

For each manual section, the user chooses one of three options:

| Option | Behavior |
|--------|----------|
| **Provide** | User supplies the content directly; stored for assembly |
| **Generate** | Template's auto-brief is dispatched to the specified agent during assembly (joins the supplemental dispatch queue). Only available when the template includes a brief for the section |
| **Skip** | Section is omitted from the assembled edition |

When the user says "just go with what you have," all manual sections default to **generate** (if a brief exists) or **skip** (if no brief).
