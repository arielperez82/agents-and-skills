...
- Use Supabase CLI: `supabase migration new <name>` and `supabase db push`
- Test locally: `supabase start` then `supabase db reset`
- Include GRANT statements for PostgREST API access
- See `databases/references/supabase-migrations.md` for complete workflow

## Index Analysis Checklist

**When reviewing migrations or schema changes, verify indexes:**
...
```sql
-- Example: Function that needs index
CREATE FUNCTION derive_entity_status(entity_uuid UUID)
RETURNS VARCHAR(20) AS $$
  SELECT status FROM thread
  WHERE contact_id IN (
    SELECT id FROM contact WHERE entity_id = entity_uuid
  )
  ORDER BY updated_at DESC  -- ⚠️ Needs index!
  LIMIT 1;
$$ LANGUAGE plpgsql;

-- ✅ CORRECT - Add index for function query
CREATE INDEX idx_thread_contact_updated ON thread(contact_id, updated_at DESC);
```

### Index Analysis Checklist

**Before approving migrations:**
- [ ] All function queries have corresponding indexes (analyze ORDER BY, WHERE clauses)
- [ ] All ORDER BY clauses have indexes (check sort columns)
- [ ] Composite indexes match query patterns (verify column order matches usage)
...
```sql
-- Pattern 1: Function query with ORDER BY
-- Function: ORDER BY updated_at DESC
-- Index needed:
CREATE INDEX idx_thread_contact_updated ON thread(contact_id, updated_at DESC);

-- Pattern 2: Composite index matching query pattern
-- Query: WHERE status = 'pending' ORDER BY due_at
-- Index needed:
CREATE INDEX idx_task_status_due ON task(status, due_at);

-- Pattern 3: Foreign key lookup
-- Query: JOIN on foreign key
-- Index needed:
CREATE INDEX idx_opportunity_entity ON opportunity(entity_id);
```
