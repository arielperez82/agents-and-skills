---
name: databases
description: Work with MongoDB (document database, BSON documents, aggregation pipelines, Atlas cloud) and PostgreSQL (relational database, SQL queries, psql CLI, pgAdmin). Includes Supabase (managed PostgreSQL with API layer). Use when designing database schemas, writing queries and aggregations, optimizing indexes for performance, performing database migrations, configuring replication and sharding, implementing backup and restore strategies, managing database users and permissions, analyzing query performance, or administering production databases.
license: MIT
---
...
- Implementing database migrations (including Supabase CLI migrations)
- Setting up replication, sharding, or clustering
- Configuring backups and disaster recovery
- Managing database users and permissions
- Analyzing slow queries and performance issues
- Administering production database deployments
- Working with Supabase (local development, migrations, PostgREST configuration)
...
### Supabase Setup (Managed PostgreSQL + API)
...
- Supabase provides managed PostgreSQL with built-in API (PostgREST)
- Use Supabase CLI for migrations (not custom tools)
- Local development via Docker containers
- Automatic API generation from schema

**See:** [supabase-migrations.md](references/supabase-migrations.md) for complete Supabase workflow.
...
- **[supabase-migrations.md](references/supabase-migrations.md)** - Supabase CLI migrations, local development, PostgREST configuration
...
- Use Supabase CLI for migrations (not custom migration runners)
- Always include GRANT statements in migrations for PostgREST access
- Test migrations locally with `supabase db reset` before pushing
- Use local Supabase instance (`supabase start`) for development and testing
- Migration files must follow `<timestamp>_name.sql` pattern in `supabase/migrations/`
- PostgREST schema cache may need reload after migrations (restart or NOTIFY)

### External Database Tools

**NocoDB:**
NocoDB is a database UI tool that connects to external databases (PostgreSQL, MySQL, etc.) as data sources. NocoDB uses a two-database architecture: a metadata DB for NocoDB's own configuration and external data sources for application data.

**Key Integration Points:**

- NocoDB connects to Supabase PostgreSQL as external data source (via UI, not `NC_DB`)
- Requires Foreign Key constraints in schema for relationship auto-detection
...
- View creation via UI, relationship auto-detection
...
**All CREATE statements must be idempotent (can run multiple times):**

```sql
-- ✅ CORRECT - Idempotent trigger creation
...
-- ✅ CORRECT - Idempotent function creation
DROP FUNCTION IF EXISTS derive_entity_status(UUID);
CREATE FUNCTION derive_entity_status(entity_uuid UUID)
RETURNS VARCHAR(20) AS $$
  -- function body
$$ LANGUAGE plpgsql;

-- ❌ WRONG - Not idempotent (fails on re-execution)
CREATE TRIGGER update_entity_updated_at ON entity ...;
```

### Rollback Script Pattern

**Use the template:** `assets/rollback-script-template.sql`

**Always include rollback section at bottom of migration:**

```sql
-- Migration: 20260107122143_triggers_functions.sql

-- Forward migration (CREATE statements)
-- ... migration content ...
...
-- DROP TRIGGER IF EXISTS update_entity_status_from_thread ON thread;
-- DROP FUNCTION IF EXISTS derive_entity_status(UUID);
...
**Document deployment steps in migration comments or README:**
...
-- 5. Verify triggers in Studio: Check trigger list
...
- [ ] All CREATE statements preceded by DROP IF EXISTS (idempotent)
- [ ] Rollback section included at bottom
- [ ] Deployment checklist documented
- [ ] Migration tested locally with `supabase db reset`
- [ ] No schema drift after migration (`supabase db diff` shows no changes)
