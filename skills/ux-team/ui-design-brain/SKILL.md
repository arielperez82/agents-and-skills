...
description: "Generate production-grade UI using real component patterns and best practices from 60+ documented interface components. Use when building web interfaces, pages, dashboards, forms, navigation, or any UI: ensures modern, minimal, SaaS-quality output grounded in design system conventions."
---

# UI Design Brain

Curated knowledge base of 60+ UI component patterns sourced from [component.gallery](https://component.gallery) and enriched with best practices, layout guidance, and usage rules. Replaces generic guessing with real design-system knowledge when generating interfaces.

**Before writing any UI code**, consult this skill to select the right components and follow their best practices. See [references/components.md](references/components.md) for the full 60-component reference.

## When to Use This Skill

Apply whenever building, designing, or generating:
...
Every generated interface should feel **modern, minimal, and production-ready**.
...
3. **One strong color moment.** Neutral palette first (warm off-whites, near-blacks, muted mid-tones). Introduce one confident accent.
...
6. **No generic AI aesthetics.** Avoid purple-on-white gradients, Inter/Roboto defaults, evenly-spaced card grids, and cookie-cutter layouts.
...
Read the request and determine which UI components are needed. Reference [references/components.md](references/components.md) to find each component by name or alias.
...
Select the style preset that best matches the intent, or ask if unclear:
...
```
Stack:       React + Tailwind CSS (unless specified otherwise)
...
Below are the 15 most commonly needed components. For the full 60+ component reference with best practices, aliases, and layout examples, see [references/components.md](references/components.md).
...
- **visual-design-foundations** — Design tokens, typography scales, color systems, spacing systems (system-level primitives)
- **ui-design-system** — Design token generation tools, component architecture patterns, developer handoff
- **web-design-guidelines** — Web Interface Guidelines compliance auditing
- **accessibility** — WCAG 2.1 AA compliance patterns
- **tailwind-configuration** — Tailwind CSS setup and customization

## Attribution

Component data sourced from [component.gallery](https://component.gallery). Best practices and layout guidance are original enrichments. MIT license.
