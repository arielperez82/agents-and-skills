...
| 6 | **A11y linting** | Accessibility in code | Yes | e.g. eslint-plugin-jsx-a11y. Part of main lint run. |
| 7 | **A11y/SEO/performance audit** | Runtime checks | No (script + optional CI) | e.g. Lighthouse. Too slow for pre-commit; run as script and optionally in CI. |
...
|----------|-------------|
| **Backlog** | "Phase 0 — Quality gate" with one ticket per item (type-check, hooks, ESLint, Prettier, MarkdownLint, a11y lint, audit script). Phase 0 includes minimal skeleton or scaffold-with-gates; feature work starts in Phase 1. |
...
| **README** | Scripts: `pnpm check`, `pnpm lint`, `pnpm lint:md`, `pnpm lighthouse`, and that pre-commit runs on every commit. |

## When generating plans or backlogs

- **Include Phase 0** as the first phase: either (1) minimal skeleton + add all seven elements, or (2) scaffold that includes quality tooling + verify/complete the gate. Then feature work in Phase 1.
- When the user says "set up a new [stack] project," remind or add: "Phase 0 = quality gate before feature work: minimal skeleton or scaffold-with-gates, then type-check, pre-commit (Husky/lint-staged), ESLint, Prettier, MarkdownLint, a11y lint, Lighthouse script; no feature work until gate is complete."
