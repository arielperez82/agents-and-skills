...
L4-L6 architecture refactoring runs after all TDD steps in a feature complete, not during each TDD inner loop. These are higher-order refactorings that benefit from seeing the full picture.

---

## Connection to TDD Refactor Phase

The RPP hierarchy integrates directly with the RED-GREEN-REFACTOR cycle:

1. **After GREEN**: Assess refactoring using L1-L6 as your priority guide
2. **L1-L2 in the inner loop**: Readability and complexity fixes happen in every REFACTOR step
3. **L3-L6 deferred**: Responsibility, abstraction, pattern, and SOLID refactorings wait until a logical feature boundary
4. **Commit before refactoring**: Working code committed first, then refactoring committed separately
5. **Tests unchanged**: All refactoring must pass existing tests without modification

The priority classification from the parent SKILL.md maps to RPP levels:

| SKILL.md Priority | RPP Level | Action |
|-------------------|-----------|--------|
| Critical (Fix Now) | L1-L2 | Mutations, knowledge duplication, deep nesting |
| High (This Session) | L2-L3 | Magic numbers, unclear names, long functions |
| Nice (Later) | L4-L6 | Abstraction refinement, patterns, SOLID |
| Skip | -- | Already clean code |
