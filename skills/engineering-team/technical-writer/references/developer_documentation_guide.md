...
**Storage:** Prefer `{ADR_DIR}/` with naming `adr-YYYYMMDD-<subject>.md` when using artifact conventions (see `LEARNINGS_FILE` per `/docs/layout`). Alternative: `docs/adr/` for repos not yet on conventions.
