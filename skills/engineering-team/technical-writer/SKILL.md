...
- **Initiative naming:** When creating or updating docs under `CANONICAL_ROOT` (per `/docs/layout`) that belong to an initiative, include `initiative` and `initiative_name` in front matter (see `LEARNINGS_FILE` initiative naming, per `/docs/layout`).
