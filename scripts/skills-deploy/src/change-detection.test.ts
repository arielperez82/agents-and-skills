...
const createDeps = (
  overrides: Partial<ChangeDetectionDeps> = {},
): ChangeDetectionDeps => ({
...
describe('getChangedSkillDirs', () => {
  it('returns empty array when no files changed', async () => {
    ...
    const result = await getChangedSkillDirs(
      { rootDir: '/repo' },
      deps,
    );
  ...
  it('maps file paths to skill directories correctly', async () => {
    const deps = createDeps({
      ...
      fileExists: (path: string) =>
        path === '/repo/skills/engineering-team/tdd/SKILL.md',
    });
  ...
  it('handles root-level skills', async () => {
    const deps = createDeps({
      ...
      fileExists: (path: string) =>
        path === '/repo/skills/agent-browser/SKILL.md',
    });
