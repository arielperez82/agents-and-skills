...
- If no argument: find most recent `{REPORTS_DIR}/report-*-craft-status-*.md` (per `/docs/layout`)
...
- Must start with `DOCS_ROOT` (per `/docs/layout`, relative) — no absolute paths
- Must not contain `..`
...
- Artifact path safety (all `artifact_paths` start with `DOCS_ROOT` per `/docs/layout`, no `..`, no absolute paths)
...
- **Path safety:** Each artifact path must pass Artifact Path Safety checks (`DOCS_ROOT` prefix per `/docs/layout`, no `..`, no absolute paths)
