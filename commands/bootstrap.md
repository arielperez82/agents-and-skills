...
2. Use `ap-implementation-planner` subagent and multiple `ap-researcher` subagents in parallel to find a best fit tech stack for this project, keeping research reports within the ≤150 lines limit.
...
* Use `ap-implementation-planner` subagent to create a detailed implementation plan following the progressive disclosure structure:
...
* Use `ui-ux-designer` subagent and multiple `ap-researcher` subagents in parallel to create a design plan that follows the same directory/phase structure described above, keeping related research reports within the ≤150 lines limit.
