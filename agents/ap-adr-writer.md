...
Invoke `ap-adr-writer` when you're **about to make** a significant architectural decision:

```markdown
user: "Should we use Redux or Zustand for state management?"
assistant: "This is a significant architectural decision affecting the entire application. Let me invoke the ap-adr-writer agent to help evaluate and document this choice."
<commentary>Technology selection with long-term impact. Invoke ap-adr-writer agent to document.</commentary>
```

### Reactive Usage

Invoke `ap-adr-writer` when you've **just made** a significant architectural decision:

```markdown
user: "I've decided we'll use BullMQ for our job queue instead of building a custom solution"
assistant: "That's an important infrastructure decision. Let me invoke the ap-adr-writer agent to document the rationale."
<commentary>Technology selection made. Document with ADR for future context.</commentary>
```

## How Other Agents Should Invoke ap-adr-writer

### For ap-architect Agent
...
**How to Invoke:**
```markdown
# In ap-architect workflow, after step 4 (Generate Architecture Diagrams):

5. **Document Architecture Decisions** - For significant architectural decisions, invoke the `ap-adr-writer` agent:
   
   **Decision Criteria**: Use the ap-adr-writer agent's decision framework:
   ...
   **Invocation Pattern:**
   ...
   # Invoke ap-adr-writer agent with decision context
   # ap-adr-writer creates structured ADR document
...
2. **Apply Decision Framework** (from ap-adr-writer):
   - Is this a one-way door?
   - Did we evaluate alternatives?
   - Will this affect future decisions?
   - Will future developers wonder "why"?

3. **If 3+ criteria met → Invoke ap-adr-writer**:
   ```markdown
   assistant: "This is a significant architectural decision. Let me invoke the ap-adr-writer agent to document the context, alternatives, and rationale."
   
   → Invoke ap-adr-writer agent
   → Provide decision context
   → ap-adr-writer creates structured ADR
...
A: "This is a significant infrastructure decision. Let me invoke the ap-adr-writer agent to help evaluate and document this choice."

[ap-adr-writer agent creates:]
...
A: "That's a foundational architectural decision. Let me invoke the ap-adr-writer agent to document it."
...
A: "That's a significant architectural decision that should have been documented. Let me invoke the ap-adr-writer agent to create a retroactive ADR based on the current codebase and rationale."
...
The `ap-adr-writer` agent has access to:
...
The `ap-adr-writer` agent is successful when:
...
```markdown
[ap-architect completes architecture design]
...
→ Invokes ap-adr-writer agent for decisions meeting criteria
→ ap-adr-writer creates structured ADR documents
...
```markdown
[ap-learn agent capturing significant learning]
→ If learning reveals architectural decision
→ Suggest invoking ap-adr-writer agent
...
### ap-adr-writer vs ap-technical-writer

**ap-adr-writer** (this agent):
...
**Relationship**: Complementary. ADRs provide decision context that complements general documentation. ap-technical-writer may reference ADRs in architecture documentation.

### ap-adr-writer vs ap-learn agent
...
**Relationship**: Complementary. ADRs explain architectural decisions, while CLAUDE.md (via ap-learn agent) captures how to work with those decisions. For example:
...
The `ap-adr-writer` agent creates Architecture Decision Records for significant architectural choices. It:
