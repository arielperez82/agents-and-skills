...
examples:
  - title: "Validate a single agent"
    input: "Validate ap-backend-engineer"
...
The `ap-agent-validator` agent validates agent specifications against the complete authoring standard. It catches YAML errors, missing fields, broken skill/agent references, incorrect classification mappings, missing body sections, and cross-reference issues.
...
`ap-agent-validator` uses a single core skill:
...
1. Run the validator on the target agent:
   ```bash
   python3 skills/agent-development-team/creating-agents/scripts/validate_agent.py agents/ap-<name>.md
   ```
...
**Steps:**
1. After modifying any `agents/ap-*.md` file, run:
   ```bash
   python3 skills/agent-development-team/creating-agents/scripts/validate_agent.py agents/ap-<name>.md
   ```
2. Or use the command: `/agent/validate ap-<name>`
...
After `ap-agent-author` creates a new agent:
```bash
# Validate the new agent
python3 skills/agent-development-team/creating-agents/scripts/validate_agent.py agents/ap-new-agent.md

# If it passes, also verify it appears in README
grep "ap-new-agent" agents/README.md
```
...
After `ap-agent-author` refactors agents:
```bash
