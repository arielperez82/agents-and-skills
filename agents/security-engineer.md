...
# === RELATIONSHIPS ===
related-agents: []
related-skills: [senior-security]
related-commands: []
collaborates-with:
  - agent: code-reviewer
...
1. **Threat Modeler**
   ...
   - **Path:** `../skills/senior-security/scripts/threat_modeler.py`
   - **Usage:** `python3 ../skills/senior-security/scripts/threat_modeler.py --input project-path --output json --verbose`
...
2. **Security Auditor**
   ...
   - **Path:** `../skills/senior-security/scripts/security_auditor.py`
   - **Usage:** `python3 ../skills/senior-security/scripts/security_auditor.py --input /path/to/codebase --output json --file security-report.json --verbose`
...
3. **Pentest Automator**
   ...
   - **Path:** `../skills/senior-security/scripts/pentest_automator.py`
   - **Usage:** `python3 ../skills/senior-security/scripts/pentest_automator.py --input target-url --config pentest-config.json --output json --verbose`
...
1. **Security Architecture Patterns**
   - **Location:** `../skills/senior-security/references/security_architecture_patterns.md`
...
2. **Penetration Testing Guide**
   - **Location:** `../skills/senior-security/references/penetration_testing_guide.md`
...
3. **Cryptography Implementation**
   - **Location:** `../skills/senior-security/references/cryptography_implementation.md`
...
1. **Generate Threat Model** - Execute threat modeler on architecture design documents and codebase
   ...
   ```bash
   python3 ../skills/senior-security/scripts/threat_modeler.py --input ./architecture-docs --output json --file threat-model.json --verbose
   ```
...
7. **Review with Stakeholders** - Present threat model to architects, developers, and product managers
   ...
   python3 ../skills/senior-security/scripts/threat_modeler.py --input ./architecture-docs --output text --summary > threat-model-summary.txt
   ```
...
1. **Execute Security Scan** - Run comprehensive security auditor across entire codebase
   ...
   ```bash
   python3 ../skills/senior-security/scripts/security_auditor.py --input ./ --output json --file security-audit.json --verbose
   ```
...
2. **Execute Authentication Testing** - Test authentication mechanisms for bypass vulnerabilities
   ...
   ```bash
   python3 ../skills/senior-security/scripts/pentest_automator.py --input pentest-config.json --test authentication --output json --file auth-test-results.json
   # Tests performed:
...
3. **Test Authorization Boundaries** - Validate authorization controls and privilege escalation risks
   ...
   ```bash
   python3 ../skills/senior-security/scripts/pentest_automator.py --input pentest-config.json --test authorization --output json --file authz-test-results.json
   # Tests performed:
...
4. **Execute Injection Attack Tests** - Fuzz inputs for SQL injection, command injection, and XSS
   ...
   ```bash
   python3 ../skills/senior-security/scripts/pentest_automator.py --input pentest-config.json --test injection --output json --file injection-test-results.json
...
5. **Test XSS Vulnerabilities** - Validate input sanitization and output encoding
   ...
   ```bash
   python3 ../skills/senior-security/scripts/pentest_automator.py --input pentest-config.json --test xss --output json --file xss-test-results.json
   # XSS types tested:
   # - Reflected XSS
   # - Stored XSS
   # - DOM-based XSS
...
6. **Validate Security Headers** - Check security header implementation (CSP, HSTS, X-Frame-Options)
   ...
   ```bash
   python3 ../skills/senior-security/scripts/pentest_automator.py --input pentest-config.json --test headers --output json --file headers-test-results.json
   # Headers checked:
...
7. **Test Rate Limiting and CSRF Protection** - Validate rate limiting and CSRF token implementation
   ...
   ```bash
   python3 ../skills/senior-security/scripts/pentest_automator.py --input pentest-config.json --test csrf --output json --file csrf-test-results.json
   # Tests performed:
...
python3 ../skills/senior-security/scripts/pentest_automator.py --input pentest-config.json --output json --file pentest-full-report.json --verbose
...
python3 ../skills/senior-security/scripts/pentest_automator.py --input pentest-config.json --retest critical-vulnerabilities.json
```
...
2. **Scan Security Controls** - Validate implementation of required security controls
   ...
   ```bash
   python3 ../skills/senior-security/scripts/security_auditor.py --input ./ --compliance SOC2 --output json --file compliance-scan.json --verbose
   # Controls validated:
...
python3 ../skills/senior-security/scripts/security_auditor.py --input ./ --compliance SOC2 --output json --file soc2-compliance.json --verbose
...
python3 ../skills/senior-security/scripts/threat_modeler.py \
...
python3 ../skills/senior-security/scripts/security_auditor.py \
  --input ./ \
  --output json \
...
python3 ../skills/senior-security/scripts/pentest_automator.py \
...
When reviewing database schemas, verify:

- [ ] RLS enabled on all application tables (100% coverage for application tables)
- [ ] RLS policies defined and tested (not just enabled)
- [ ] Security tests verify RLS enforcement for all operations (SELECT, INSERT, UPDATE, DELETE)
- [ ] Service role access verified (where service role is required)
- [ ] Anon/authenticated access blocked appropriately (where required)
- [ ] RLS policy strategy documented (see [`supabase/migrations/RLS_POLICY_STRATEGY.md`](../../supabase/migrations/RLS_POLICY_STRATEGY.md))

**Security Testing Coverage:**

- [ ] Write operation security tests present (INSERT/UPDATE/DELETE, not just SELECT)
- [ ] RLS enforcement verified for all operations
- [ ] Service role write operation tests explicit (not just implicit)
- [ ] Anon client write operation tests verify blocking
- [ ] Security tests use actual Supabase clients (not mocks)

**Flag Missing RLS as CRITICAL (P0):**

- Missing RLS on application tables = CRITICAL security issue
- Require RLS coverage = 100% for application tables before production
- Do not approve migrations with missing RLS on application tables
...
- [architect](architect.md) - Provides system design context for security architecture decisions and threat modeling
- [devops-engineer](devops-engineer.md) - Collaborates on DevSecOps practices, security pipeline integration, and infrastructure security
- [backend-engineer](backend-engineer.md) - Works together on API security, authentication implementation, and secure coding practices
- [fullstack-engineer](fullstack-engineer.md) - Coordinates on end-to-end application security including frontend security controls
- [code-reviewer](code-reviewer.md) - Integrates security review into code review process for secure coding validation

## References

- **Skill Documentation:** [../skills/senior-security/SKILL.md](../skills/senior-security/SKILL.md)
- **Domain Guide:** [../skills/CLAUDE.md](../skills/CLAUDE.md)
