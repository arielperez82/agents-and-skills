...
In addition to structural validation, the skill-validator performs lightweight content safety checks on SKILL.md frontmatter string fields. These checks run inline (no external dependencies) and complete in <100ms.

### Invisible Unicode Characters

Scan all frontmatter string fields (name, description, tags, use-cases entries, etc.) for invisible Unicode characters that could be used to hide prompt injection payloads:
...
**Report format:** Flag as CRITICAL with the field name, character code point, and byte offset. These are always errors because invisible characters have no legitimate purpose in skill frontmatter.
...
**Report format:** Flag as HIGH (warning) with the field name and matched pattern. These are warnings because in rare cases a description might legitimately reference these terms, but they should be reviewed.
...
- **Content safety:** Zero invisible Unicode characters or unreviewed role markers in committed skill specs.
