...
related-skills: [engineering-team/avoid-feature-creep, engineering-team/senior-architect, engineering-team/technical-writer, engineering-team/core-testing-methodology, engineering-team/docker-compose-basics, engineering-team/terraform-configuration, engineering-team/monorepo-architecture]
related-commands: []
collaborates-with:
...
examples:
  ...
  - title: "Generate Architecture Diagram"
    ...
    output: "Mermaid diagram generated via ap-technical-writer collaboration"
...
3. **Design System Components** - Define core architecture components:
   ...
   - API layer (REST, GraphQL, gRPC) - when GraphQL is selected, invoke ap-graphql-architect for detailed schema and federation design
   - Backend services (Node.js/Express, Go, Python)
   - Data layer (PostgreSQL, Redis, message queues) - when Supabase is selected, invoke ap-supabase-database-engineer for schema design, RLS policies, and migration management
...
5. **Document Architecture Decisions** - For significant architectural decisions, invoke the `ap-adr-writer` agent:
   
   **Decision Criteria**: Use the ap-adr-writer agent's decision framework to determine if a decision merits an ADR:
   ...
   **Invocation Pattern:**
   ...
   3. Invoke ap-adr-writer agent with decision context
   4. ap-adr-writer creates structured ADR document following standard format
...
# Invoke ap-adr-writer agent for significant decisions, then present to stakeholders
```
...
2. **Research Technology Options** - Review reference guide:
   ...
   - **API**: REST vs GraphQL vs gRPC - if GraphQL is selected, invoke ap-graphql-architect for detailed evaluation
...
5. **Create Migration Roadmap** - Plan phased approach:
   ...
   - **Phase 2**: Migrate high-traffic services (scale validation) - if GraphQL services, invoke ap-graphql-architect for federation planning
...
- [ap-adr-writer](ap-adr-writer.md) - **Invoked for ADR creation** - Documents significant architectural decisions with context, alternatives, and rationale. ap-architect invokes ap-adr-writer when decisions meet ADR criteria (one-way door, alternatives evaluated, foundational, future developers will wonder why)
- [ap-cto-advisor](../c-level/ap-cto-advisor.md) - Provides strategic technical leadership and technology vision that guides architecture decisions
- [ap-technical-writer](ap-technical-writer.md) - **Collaborates for Mermaid diagram generation** (architecture, sequence, class, ERD diagrams)
- [ap-graphql-architect](ap-graphql-architect.md) - **Invoked for GraphQL architecture** - When GraphQL is selected as API technology, ap-architect delegates detailed schema design, federation planning, and GraphQL-specific performance optimization to ap-graphql-architect
- [ap-supabase-database-engineer](ap-supabase-database-engineer.md) - **Invoked for Supabase database design** - When Supabase is selected as the database solution, ap-architect delegates schema design, RLS policy architecture, and migration management to ap-supabase-database-engineer
- [ap-backend-engineer](ap-backend-engineer.md) - Implements backend services following architecture patterns and guidelines
- [ap-frontend-engineer](ap-frontend-engineer.md) - Implements frontend applications following architecture patterns and component design
- [ap-fullstack-engineer](ap-fullstack-engineer.md) - Implements end-to-end features following full-stack architecture patterns
- [ap-devsecops-engineer](ap-devsecops-engineer.md) - DevSecOps implementation of secure infrastructure and deployment pipelines based on architecture design
- [ap-security-engineer](ap-security-engineer.md) - Validates security patterns and implements security requirements in architecture

## Collaboration: Architecture Diagram Generation

**For architecture diagrams, ap-architect delegates to ap-technical-writer's Mermaid Diagram Generator.**

This follows the complementary design principle - architecture design stays with ap-architect, while diagram generation is a technical-writer capability.

### Collaboration Workflow

1. **ap-architect designs** the system architecture (components, layers, data flow)
2. **ap-architect creates** JSON/YAML definition of the architecture
3. **ap-technical-writer generates** Mermaid diagrams using mermaid_diagram_generator.py
4. **ap-architect reviews** and embeds diagrams in documentation

### Example: Microservices Architecture Diagram

**Step 1: ap-architect creates architecture definition**
...
**Step 2: ap-technical-writer generates diagram**
