...
collaborates-with:
  ...
  - agent: legacy-codebase-analyzer
...
examples:
  ...
  - title: "Generate Architecture Diagram"
    ...
    output: "Mermaid diagram generated via technical-writer collaboration"
...
3. **Design System Components** - Define core architecture components:
   ...
   - API layer (REST, GraphQL, gRPC) - when GraphQL is selected, invoke graphql-architect for detailed schema and federation design
...
2. **Research Technology Options** - Review reference guide:
   ...
   - **API**: REST vs GraphQL vs gRPC - if GraphQL is selected, invoke graphql-architect for detailed evaluation
...
5. **Create Migration Roadmap** - Plan phased approach:
   ...
   - **Phase 2**: Migrate high-traffic services (scale validation) - if GraphQL services, invoke graphql-architect for federation planning
...
# Configuration
PROJECT_PATH="."
ASSESSMENT_DIR="{CANONICAL_ROOT}/assessments"
# When creating roadmap/backlog/plan under {CANONICAL_ROOT}/, include initiative and initiative_name in front matter (see LEARNINGS_FILE initiative naming).
...
- [adr-writer](adr-writer.md) - **Invoked for ADR creation** - Documents significant architectural decisions with context, alternatives, and rationale. architect invokes adr-writer when decisions meet ADR criteria (one-way door, alternatives evaluated, foundational, future developers will wonder why)
- [cto-advisor](../c-level/cto-advisor.md) - Provides strategic technical leadership and technology vision that guides architecture decisions
- [technical-writer](technical-writer.md) - **Collaborates for Mermaid diagram generation** (architecture, sequence, class, ERD diagrams)
- [graphql-architect](graphql-architect.md) - **Invoked for GraphQL architecture** - When GraphQL is selected as API technology, architect delegates detailed schema design, federation planning, and GraphQL-specific performance optimization to graphql-architect
- [supabase-database-engineer](supabase-database-engineer.md) - **Invoked for Supabase database design** - When Supabase is selected as the database solution, architect delegates schema design, RLS policy architecture, and migration management to supabase-database-engineer
...
- [devsecops-engineer](devsecops-engineer.md) - DevSecOps implementation of secure infrastructure and deployment pipelines based on architecture design
...
**For architecture diagrams, architect delegates to technical-writer's Mermaid Diagram Generator.**
...
3. **technical-writer generates** Mermaid diagrams using mermaid_diagram_generator.py
...
**Step 2: technical-writer generates diagram**
