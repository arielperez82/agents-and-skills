...
In addition to structural validation, the agent-validator performs lightweight content safety checks on frontmatter string fields. These checks run inline (no external dependencies) and complete in <100ms.
...
**Report format:** Flag as CRITICAL with the field name, character code point, and byte offset. These are always errors because invisible characters have no legitimate purpose in agent frontmatter.
...
- **Content safety:** Zero invisible Unicode characters or unreviewed role markers in committed agent specs
