...
1. **Load voice profile** — Read the publication's `.voice-profile.md` file (generated by `/voice/extract`). Falls back to raw reference editions if no profile available.
