...
**IMPORTANT**: If failures suggest over-implementation or unclear “minimum code” choices, recommend invoking `tpp-guardian` to select the next smallest change to satisfy the next test and no more.
...
4. Analyze test results, paying special attention to failures
   - If the failure indicates the implementation is too broad/complex for the current test intent, suggest a TPP-driven next step (`tpp-guardian`) to narrow to “just enough code”
...
**Schema Migration Test Coverage Checklist:**
When reviewing schema migrations, verify:
- [ ] All UNIQUE constraints tested (including single-row patterns like configuration tables)
- [ ] All triggers tested (every trigger created in migration has corresponding test)
- [ ] All CHECK constraints have edge case tests (invalid values, boundary conditions)
- [ ] Security tests cover read AND write operations (INSERT/UPDATE/DELETE, not just SELECT)
- [ ] Branch coverage ≥ 90% (flag if below threshold)
- [ ] All foreign key relationships tested
- [ ] All indexes tested via query patterns (verify index effectiveness)
- [ ] RLS enforcement verified for all operations (if RLS is enabled)
