...
| 6 | Hook wrappers configured and E2E verified | 5 pre-compiled JS entry-point wrappers under .claude/hooks/; .claude/settings.local.json updated with hook definitions; all hooks log failures to telemetry_health; E2E verification passes with specific pass/fail checklist |
| 7 | Native OTel enabled and documented | CLAUDE_CODE_ENABLE_TELEMETRY configured with HTTPS-only Tinybird OTLP endpoint; setup documented in .docs/; standard metrics verified flowing to Tinybird |
| 8 | Feedback loop verified, docs updated, production deployed | SessionStart hook queries agent_usage_summary with 2s timeout and cache fallback, returns validated optimization context; CLAUDE.md updated with telemetry-informed agent selection guidance (mandatory agents stay mandatory); .docs/AGENTS.md updated with I05-ATEL references, learnings, and agent name stability note; Tinybird deployed to production |
