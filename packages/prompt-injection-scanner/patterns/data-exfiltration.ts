  ...
  rules: [
    {
      id: 'de-001',
      pattern:
        /\b(send|exfiltrate|leak|transmit)\b.*\b(contents?|data|prompt|response)\b.*\b(to|via)\b.*\b(https?:|url|server|endpoint)/i,
      severity: 'CRITICAL',
    ...
    },
    {
      id: 'de-004',
      pattern:
        /\b(read|cat|access)\b.*(?:\/etc\/|~\/\.|\.ssh|\.env\b|credentials|passwd|shadow|private.?key).*\b(forward|send|share|post|upload)\b/i,
      severity: 'CRITICAL',
    ...
    },
    {
      id: 'de-005',
      pattern:
        /\b(upload|post|transmit|exfil)\b.*\b(data|content|file|secret|token)\b.*\b(external|remote|outside|server)\b/i,
      severity: 'CRITICAL',
