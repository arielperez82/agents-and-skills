...
export const safetyBypass: PatternCategory = {
  ...
  rules: [
    ...
    },
    {
      id: 'sb-004',
      pattern:
        /\bact\s+(without|with\s+no)\s+(any\s+)?(restrictions?|limitations?|constraints?|boundaries)\b/i,
      severity: 'HIGH',
    ...
    },
    {
      id: 'sb-005',
      pattern:
        /\bpretend\s+(you\s+)?(have\s+no|don'?t\s+have|are\s+free\s+from)\s+(rules?|guidelines?|restrictions?|limits?)\b/i,
      severity: 'HIGH',
