#!/bin/bash
# Install review-nudge hooks via symlinks.
#
# Usage:
...
LINKS=(
  "$SCRIPTS_SRC/review-nudge-post.sh:$HOOKS_DIR/review-nudge-post.sh"
  "$SCRIPTS_SRC/review-nudge-cleanup.sh:$HOOKS_DIR/review-nudge-cleanup.sh"
)

check_mode() {
  echo "Review Nudge - symlink check"
  echo ""
...
install_mode() {
  echo "Installing review-nudge symlinks..."
  ...
  echo ""
  echo "Optional: set environment variables to tune behavior:"
  echo "  REVIEW_NUDGE_THROTTLE=60   # seconds between nudges (default: 60)"
}

uninstall_mode() {
  echo "Removing review-nudge symlinks..."
