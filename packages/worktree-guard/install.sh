#!/bin/bash
# Install worktree-guard hooks via symlinks.
#
# Usage:
...
LINKS=(
  "$SCRIPTS_SRC/worktree-guard-pre.sh:$HOOKS_DIR/worktree-guard-pre.sh"
)

check_mode() {
  echo "Worktree Guard - symlink check"
  echo ""
...
install_mode() {
  echo "Installing worktree-guard symlinks..."
...
uninstall_mode() {
  echo "Removing worktree-guard symlinks..."
