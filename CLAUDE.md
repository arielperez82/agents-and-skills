...
**Context Awareness:** Before starting any new unit of work in a multi-step workflow, check context utilization. Quick check: `echo '{"context_window":{"used_percentage":N}}' | ~/.claude/scripts/status-line.sh --agent` (returns zone: `OK`/`CAUTION`/`STOP`/`BLOCKED`). If at 40%+ (`CAUTION`), prioritize handoff over starting new work. Use `/context/handoff` — it auto-detects whether to update an active craft status file or write a standalone handoff to `.docs/reports/handoff-{context}-{timestamp}.md`. A clean handoff at 55% is always better than degraded output at 80%.
