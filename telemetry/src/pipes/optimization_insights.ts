    ...
    node({
      ...
      sql: `
        SELECT
          ...
          count() AS frequency,
          if(sum(input_tokens + output_tokens) > 0,
            sum(cache_read_tokens) / sum(input_tokens + output_tokens), 0) AS cache_ratio,
          if(sum(input_tokens + output_tokens) > 0,
            (sum(cache_read_tokens) / sum(input_tokens + output_tokens)) * log(count() + 1), 0) AS efficiency_score
